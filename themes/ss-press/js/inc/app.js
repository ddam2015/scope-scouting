...
var ogp_reveal_closer_today_button = $('#reveal_close_today');
if (ogp_reveal_closer_today_button.length > 0) {
  ogp_reveal_closer_today_button.on('click', function() {
    localStorage.setItem("ogp_close_today", 'true');
    localStorage.setItem("ogp_close_today_date", new Date() );
  });
}
...
$('#event-menu-region').append('<div class="button auto-margin-sides"><a href="https://grassroots365.com/calendar/" class="show-for-medium-up" id="eventAllBtnBtm">see all events</a></div>');
...
//     const tableEl = document.querySelector('.club-rosters table');
//     const thEls = tableEl.querySelectorAll('.club-rosters thead th');
//     const tdLabels = Array.from(thEls).map(el => el.innerText);
//     tableEl.querySelectorAll('.club-rosters tbody tr').forEach( tr => {
