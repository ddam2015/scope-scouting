...
<form id="wc_cart_form" class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
        ...
        $num = $_product->get_price();
        $number = (int)$num;
        

				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) && $number > 0  ) {
          ...
          if( !empty(WC()->cart->get_cart()) ){
            ...
            foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ){
              ...
              if( in_array($product_cat_slug, $extra_types) ){
                ...
                echo '<td></td>';
                echo '<td>'; _e( 'A 2.9% merchant fee is applied to all credit card transactions', 'woocommerce' ); echo '<span class="min-width text-left in-block tiny-padding-left"></span></td>';
                echo '<td colspan="6" class="text-right"><span>'; echo '$'.round($percentage_fee, 2); echo '</span></td>';
              }
            }
          }
        ?>
