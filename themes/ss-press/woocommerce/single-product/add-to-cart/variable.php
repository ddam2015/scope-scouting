...
<form class="variations_form cart cell small-12" action="<?php echo esc_url( get_permalink() ); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo htmlspecialchars( wp_json_encode( $available_variations ) ) ?>">
	...
	<?php else : ?>
      ...
      <?php foreach ( $attributes as $attribute_name => $options ) : 
        ...
        } else { //default dropdown layout
          echo '<div class="input-group max-button ' . (( count($attributes) == 1 || $attribute_name === array_keys($attributes)[count($attributes)-1] ) ? "no-margin" : "small-margin-bottom") . '">';
          echo '<label class="input-group-label" for="' . esc_attr( sanitize_title( $attribute_name ) ) . '">' . wc_attribute_label( $attribute_name ) . '</label>';
