...
function pl_waiver_columns( $columns ) {
  ...
  $columns['ogp_user_name'] = 'User Name';
  $columns['ogp_user_email'] = 'User Email';
  return $columns;
}
add_filter( 'manage_nf_sub_posts_columns', 'pl_waiver_columns' );

function pl_waiver_column( $column, $sub_id ) {
  if( $column === 'ogp_user_name' || $column === 'ogp_user_email' ) {
    //get submission data
    $sub = Ninja_Forms()->form()->get_sub( $sub_id );
    //pull user info
    $user = $sub->get_user();
    //add it in to the column
    if( $column === 'ogp_user_name') echo $user->data->user_nicename;
    if( $column === 'ogp_user_email') echo $user->data->user_email;
  }
}
...
function add_apple_touch_icons(){
	...
	echo '<link rel="icon" type="image/png" href="http://scopescouting.com/wp-content/uploads/2024/07/cropped-Scope-Scouting-Icon-400x300-1.png" />' . "\n";
...
if ( ! function_exists( 'theme_setup' ) ) :
    ...
    add_theme_support( 'align-wide' );
		...
		load_theme_textdomain( 'ss-press' );
...
if ( ! function_exists( 'ogp_main_nav' ) ) {
	function ogp_main_nav() {
    ...
    wp_nav_menu( array(
			...
			'fallback_cb'    => false,
			'walker'         => new ogp_Top_Bar_Walker()
		));
	}
}
// Main Navigation Mega
if ( ! function_exists( 'ogp_mega_nav' ) ) {
	function ogp_mega_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new ogp_Mega_Walker()
		));
	}
}
// Main Navigation side Slide
if ( ! function_exists( 'ogp_side_slide_nav' ) ) {
	function ogp_side_slide_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new ogp_Side_Slide_Walker()
		));
	}
}
...
if ( ! function_exists( 'ogp_new_dropdown_nav' ) ) {
	function ogp_new_dropdown_nav() {
    function mobile_menu_support($classes, $item, $args) {
      $classes[] = ( in_array('non-mobile', $classes) ) ? 'hide' : 'hide-for-medium';
      return $classes;
    }
...
// Title Nav
if ( ! function_exists( 'ogp_title_nav' ) ) {
	function ogp_title_nav() {
		wp_nav_menu( array(
			'theme_location' => 'title_nav',
			'container'      => false,
...
// Footer Nav
if ( ! function_exists( 'ogp_footer_nav' ) ) {
	function ogp_footer_nav() {
		wp_nav_menu( array(
...
if ( ! function_exists( 'ogp_excerpt' ) ) :
	/**
	 * Displays the optional excerpt.
	 * Wraps the excerpt in a div element.
	 * Create your own ogp_excerpt() function to override in a child theme.
	 * @param string $class Optional. Class string of the div element. Defaults to 'entry-summary'.
	 */
	function ogp_excerpt( $class = 'entry-summary' ) {
...
if ( ! function_exists( 'ogp_excerpt_more' ) && ! is_admin() ) :
/**
 * Replaces "[...]" (appended to automatically generated excerpts) with ... and a 'Continue reading' link.
 * Create your own ogp_excerpt_more() function to override in a child theme.
 * @return string 'Continue reading' link prepended with an ellipsis.
 */
function ogp_excerpt_more() {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'ss-press' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'ogp_excerpt_more' );
endif;

if ( ! function_exists( 'ogp_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 * Create your own ogp_entry_meta() function to override in a child theme.
 */
function ogp_entry_meta() {
	if ( 'posts' === get_post_type() || 'careers_template' === get_post_type() ) {
		$author_avatar_size = apply_filters( 'ogp_author_avatar_size', 49 );
		printf( '<span class="byline"><span class="author vcard">%1$s <span class="screen-reader-text">%2$s </span> <a class="url fn n" href="%3$s">%4$s</a></span></span>',
			get_avatar( get_the_author_meta( 'user_email' ), $author_avatar_size ),
			_x( 'Author', 'Used before post author name.', 'ss-press' ),
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'ss-press' ) ),
...
// 		ogp_entry_taxonomies();
	}

	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'ss-press' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;

if ( ! function_exists( 'ogp_entry_date' ) ) :
/**
 * Prints HTML with date information for current post.
 * Create your own ogp_entry_date() function to override in a child theme.
 */
function ogp_entry_date() {
...
// 		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time> | <time class="updated" datetime="%3$s">%4$s</time>';
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'ss-press' ),
...
function ogp_admin_menu() {
	add_menu_page( 'OGP Data Admin', 'Data', 'manage_options', 'ogp_data', 'ogp_admin', 'dashicons-chart-line', 26  );
// 	add_submenu_page( 'ogp_data', 'ogp Data Dashboard', 'Dashboard', 'manage_options', 'ogp_data', 'ogp_admin' );
}
add_action( 'admin_menu', 'ogp_admin_menu' );

//build admin page
function ogp_admin(){
	wp_enqueue_style( 'ogp_admin_styles', get_stylesheet_directory_uri() . '/css/ogp-admin-style.css' );
	...
	$ogp_sections = array(
		'settings' => array(
			...
			'tab_name'	=> 'Settings',
			'url'			=> '?page=ogp_data&g_action=settings'
		)
	);
	echo '<div class="ogp_data_manager_wrapper">';
	$ogp_admin_url = '';
	$nav = '';
	foreach( $ogp_sections as $section => $section_data ) {
		if( $section == $g_action ) {
			echo '<h1>' . $section_data['title'] . '</h1>';
			$ogp_admin_url = $section_data['url'];
	...
	echo '<nav class="nav-tab-wrapper">' . $nav . '</nav><div class="ogp_data_manager_content_wrapper">';
	switch( $g_action ) {
		case 'export_data':
			ogp_export();
			break;
		...
		case 'settings':
			$ogp_admin_settings = array(
				...
				'layout' => array(
                  ...
                  'options' => array(
                    ...
                    array( 'option_name' => '2022 Menu', 'option' => 'new' )
                  )
								)
							)
						)
			...
			function ogp_update_admin_site_settings( $old, $new ) {
				foreach( $new as $item => $item_data ) {
					foreach( $item_data as $item_name => $item_value ) {
						$old[$item]['items'][$item_name]['value'] = ogp_process_data_point( $old[$item]['items'][$item_name]['type'], $item_value);
            if( $old[$item]['items'][$item_name]['value'] === null || $old[$item]['items'][$item_name]['value'] === 'null' ) $old[$item]['items'][$item_name]['value'] = '';
					}
				}
				return $old;
			}
			foreach( $ogp_admin_settings as $setting_set => &$setting_data ) {
				if( $_POST['g_process'] == 'process' ) {
					$ogp_db_option = $_POST['ogp_admin_form_data'][$setting_set];
					$update = update_option( 'ogp_' . $setting_set, $ogp_db_option );
					$setting_data['section_records'] = ogp_update_admin_site_settings( $setting_data['section_records'], $ogp_db_option );
					if( $update === true ) echo '<p class="success">Updated settings data!</p>';
				} else {
					$ogp_db_option = get_option( 'ogp_' . $setting_set );
					if( $ogp_db_option !== false && !empty($ogp_db_option) ) $setting_data['section_records'] = ogp_update_admin_site_settings( $setting_data['section_records'], $ogp_db_option );
        }
			} ?>
			<h3>General Site Settings</h3>
			<form id="ogp_form" method="post" class="ogp_form" action="<?php echo $ogp_admin_url; ?>">
				<input type="hidden" name="g_process" value="process" />
			<?php foreach( $ogp_admin_settings as $setting_set_re => $setting_data_re ) : ?>
				...
				<table class="ogp_form_section form-table">
					<tbody>
					<?php foreach( $record_data['items'] as $element => $element_data ) {
						$element_data['tag'] = 'ogp_admin_form_data[' . $setting_set_re . '][' . $record . '][' . $element . ']';
						$element_data['description'] = ( empty($element_data['description']) ) ? '' : '<small>' . $element_data['description'] . '</small>';
						echo ogp_template_construction( $element_data );
					} ?>
					</tbody>
				</table>
				<?php endforeach;
...
function send_html_email($email, $subject, $message) {
  $headers = ( strpos( site_url(), 'dev' ) !== false ) ? array('From: OGP Dev Customer Service <no-reply@dev.opengympremier.com>') : array('From: OGP Customer Service <no-reply@opengympremier.com>');
...
// 	$pageID = ( $pageID === null ) ? $post->ID : $pageID;
	...
	$ogp_site_ads = get_option( 'ogp_display' );
	//if the page doesn't have an ad or the site global is empty, don't add to array
	$ad_info['go'] = ( empty($ogp_page_ad['link']) || empty($ogp_page_ad['img']) || empty($ogp_site_ads) ) ? false : true;
	...
	if ( $ad_info['go'] ) {
		...
		$ogp_site_ads['site_1']['element_type'] = 'rotator';
		$ogp_site_ads['site_2']['element_type'] = 'rotator';
		//build rotator element for page ad
		$ad_info['ad_content'] .= ogp_template_construction( $ogp_page_ad );
		//if we have a global ad, build and add it
		if( !empty($ogp_site_ads['site_1']['img']) ) $ad_info['ad_content'] .= ogp_template_construction( $ogp_site_ads['site_1'] );
		//if we have a secondary global ad, build if and add it along with another primary ad to help the flow
		if( !empty($ogp_site_ads['site_2']['img']) ) {
			$ad_info['ad_content'] .= ogp_template_construction( $ogp_page_ad );
			$ad_info['ad_content'] .= ogp_template_construction( $ogp_site_ads['site_2'] );
		}
	}
	//on the front page, and if we have a global splash setting
	if( is_front_page() && !empty($ogp_site_ads['site_3']) ) {
		//controls for the splash
		$ogp_splash = array(
			'title'				=> ( empty($ogp_site_ads['site_3']['title']) ) ? '' : $ogp_site_ads['site_3']['title'],
			'link'				=> ( empty($ogp_site_ads['site_3']['link']) ) ? '' : $ogp_site_ads['site_3']['link'],
			'img'					=> ( empty($ogp_site_ads['site_3']['img']) ) ? '' : $ogp_site_ads['site_3']['img'],
			'img_mobile'	=> ( empty($ogp_site_ads['site_3']['img_mobile']) ) ? '' : $ogp_site_ads['site_3']['img_mobile'],
			'description'	=> ( empty($ogp_site_ads['site_3']['description']) ) ? '' : $ogp_site_ads['site_3']['description']
		);
		//if we have minimum info process the splash
		if( !empty($ogp_splash['title']) && !empty($ogp_splash['link']) && !empty($ogp_splash['img']) ) {
			//build the splash html
			$ad_info['splash'] = '<div class="reveal text-center" id="ogp_home_reveal" aria-labelledby="' . $ogp_splash['title'] . '" data-reveal><div class="relative"><h1 id="reveal-title" class="show-for-sr">' . $ogp_splash['title'] . '</h1>';
			//if there is a derscription, add it
			if( !empty($ogp_splash['description']) ) $ad_info['splash'] .= '<p class="reveal-description">'. $ogp_splash['description'] . '</p>';
			//if there is a mobile image, add it
			if( !empty($ogp_splash['img_mobile']) ) $ad_info['splash'] .= '<p class="text-center show-for-small-only"><a href="' . $ogp_splash['link'] . '"><img src="'. $ogp_splash['img_mobile'] . '" alt="'. $ogp_splash['title'] . '" title="'. $ogp_splash['title'] . '" /></a></p>';
			//if there is a mobile image, add supporting class to the large image
			$mobile_img_class = ( empty($ogp_splash['img_mobile']) ) ? '' : ' show-for-medium';
			//add main splash image
			$ad_info['splash'] .= '<p class="text-center' . $mobile_img_class . '"><a href="' . $ogp_splash['link'] . '"><img src="'. $ogp_splash['img'] . '" alt="'. $ogp_splash['title'] . '" title="'. $ogp_splash['title'] . '" /></a></p>';
...
function ogp_template_construction( $ele_data ) {
...
function ogp_process_data_point( $name, $point ) {
...
function ogp_build_dates($dates, $type = 1, $abbv = false, $add_reg = false) {
    ...
    switch( $type ){
      ...
      case 'date':
        ...
        if( empty($event_object->dates) ) {
          ...
          $date_reg = ogp_build_dates($event_object->dates, 2, false, $reg_days);
          if( $reg_days === false ) {
      ...
      case 'full_address':
        if( empty($event_object->short_locations) ) {
          ...
          $reg_days = $atts['reg_days'];
          $date_reg = ogp_build_dates($event_object->short_locations, 2, false, $reg_days);
          if( $reg_days === false ) {
            $data_compile .= $date_reg;
          } else {
            $data_compile .= $date_reg[0];
...
function spp_player_info_build( $player_info = null){
...
add_shortcode( 'spp_player_info', 'spp_player_info_build' );


function spp_team_info_build( $player_info = null){
  $pl_inf = spp_fn(['fn_name'=>'spp_grab_info', 'arguments'=>['team_info', ['target_url'=>get_site_url(), 'season' => 'frnt_page']]]); //this should grab the function message.....hopefully.
  $pl_inf = json_decode(json_encode($pl_inf), true);
//   echo '<br><br><br>' . print_r($pl_inf['query_result']);
  $return_string = '';
...
//       $stat_trends = json_decode($result['stat_trends'], true);
//       $display = $stat_trends['front_page'];
//       echo 'testing: ' . print_r($result) . ' // ' . $display . ' <br>';
      $brand = $result['event_brand'];
      $event_name = $result['event_name'];
      ...
      $return_string = '
        <ul class="grid-y eval-list eval-list--teams">
          <h2>Teams</h2> ';
      ...
      foreach($grouped_results as $brand => $events_by_name) {
        ...
        foreach($events_by_name as $event_name => $events) {
          foreach($events as $result) {
            ...
            $return_string .= '<li class="eval-listItem">
                                  <div class="grid-x grid-y-small">
                                    <img src="' . (!empty(esc_html($result['org_img'])) ? 'https://sportspassports.com/wp-content/uploads/org-logos/'.esc_html($result['org_img']) : 'https://dev.sportspassports.com/wp-content/uploads/event-profiles/'.'g365_profile_placeholder.gif' ) . '" alt="" class="">
                                  </div>
                                  <div class="eval-full-text">
                                    <div class="eval-info">';
               $return_string .= '   <h3>' . esc_html($result['org_name']) . '<span class="hide-for-small-only"> | '. esc_html($result['team_name']) . ' | '. esc_html($result['org_city']) . ', ' . esc_html($result['org_state']) .'</span> </h3>';
              $return_string .= '   <p class="show-for-small-only">' . esc_html($result['team_name']) . ' </p>';
              $return_string .= '   <p class="team-event show-for-small-only" data-event-brand = "'. esc_html($result['event_brand']) .'" data-full-event-name="' . esc_html($result['event_name']) . '">' . esc_html($result['org_city']) . ', ' . esc_html($result['org_state']) . ' </p>';
      //         $return_string .= '   <p class="player-event" data-full-event-name="' . esc_html($result['event_name']) . '">' . (!empty($team_search) ? esc_html($team_search) : ' ') . ' </p>';
              $return_string .= ' </div>';
              $return_string .= ' <p>' . ($result['team_eval']) . ' </p>
                                  </div>
                                  <button class="read-eval-btn">read more</button>
                                 </li>';
              $count++;
...
add_shortcode( 'spp_team_info', 'spp_team_info_build' );

function spp_event_info_build( $player_info = null){
  $today = date("Y-m-d");
  $pl_inf = spp_fn(['fn_name'=>'spp_grab_info', 'arguments'=>['event_info', ['target_url'=>get_site_url(), 'today_date'=>$today]]]); //this should grab the function message.....hopefully.
  $pl_inf = json_decode(json_encode($pl_inf), true);
//   echo '<br> ' . $pl_inf['message'] . ' <br><br>' . print_r($pl_inf['query_result']);
//   echo 'here ' . $today;
  ...
  if (isset($pl_inf['query_result']) && is_array($pl_inf['query_result'])) {
  $return_string = '
      <ul class="grid-y eval-list eval-list--events">
        <h2>Events</h2> ';
  foreach ($pl_inf['query_result'] as $result){
    $date_str = array_shift(explode('|', $result['dates']));

    
    
    $return_string .= '<li class="eval-listItem">
                        <div class="grid-x grid-y-small">
                            <img src="' . esc_html($result['logo_img']) . '" alt="" class="">
                        <div class="eval-info">';
    $return_string .= ' <h3>' . esc_html($result['name']) . '</h3>
                        </div>
                        </div>';
    $return_string .= '<div class="eval-full-text">
                         <p>' . esc_html($result['locations']) . '</p>
                       </div>
                       <p class="eval-event-date">'. $date_str . '</p>
                       </li>';
...
add_shortcode( 'spp_event_info', 'spp_event_info_build' );
