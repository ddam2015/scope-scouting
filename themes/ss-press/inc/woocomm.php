...
if ( function_exists( 'is_woocommerce' ) ) {
  ...
  //woocommerce wrappers
	...
	function ogp_wrapper_start() {
		echo '<section id="content" class="grid-x site-main woocomm-wrap" role="main">';
		echo '<div class="cell small-12 large-padding">';
	}
	function ogp_wrapper_end() {
		echo '</div>';
		echo '</section>';
	}
	add_action('woocommerce_before_main_content', 'ogp_wrapper_start', 10);
	add_action('woocommerce_after_main_content', 'ogp_wrapper_end', 10);
	...
	function ogp_woocommerce_image_dimensions() {
		global $pagenow;
	...
	add_action( 'after_switch_theme', 'ogp_woocommerce_image_dimensions', 1 );

	//remove the breadcrumb (on product pages)
	function ogp_remove_wc_breadcrumbs() {
			remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
	}
	add_action( 'init', 'ogp_remove_wc_breadcrumbs' );
  ...
  add_action( 'woocommerce_before_cart', 'woocommerce_checkout_login_form' );
	...
	function ogp_place_order_button_text() { return __( 'Complete Registration', 'woocommerce' ); }
	add_filter( 'woocommerce_order_button_text', 'ogp_place_order_button_text' );
  ...
  //change add to cart button text
	function ogp_woocommerce_product_add_to_cart_text(){
		return 'Add to Cart';
	}
	add_filter( 'woocommerce_product_add_to_cart_text' , 'ogp_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'ogp_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_booking_single_add_to_cart_text', 'ogp_woocommerce_product_add_to_cart_text' );
...
<!--       <h1>Player Manager</h1> -->
    ...
    if( !empty($user_g365) && isset($user_g365['pl_ed']) && !empty($user_g365['pl_ed']) ) {
      if( strpos(site_url(), 'dev.') === false ) {
        wp_enqueue_script( 'js-g365-all', 'https://sportspassports.com/data-processor.js', array('jquery'), '695450', true );
      } else {
        wp_enqueue_script( 'js-g365-all', 'https://dev.sportspassports.com/data-processor.js', array('jquery'), '695450', true );
      }
      ?>
      <div>
...
function ogp_wc_scripts_styles() {
	if( is_admin() ) return;
  wp_dequeue_style( 'stripe_styles' );
  // js helpers for the cart page
  if( is_cart() || is_checkout() ) {
    wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://sportspassports.com/data-processor.js' : 'https://dev.sportspassports.com/data-processor.js'), array('jquery'), '695450', true );
  }
}
add_action( 'wp_enqueue_scripts', 'ogp_wc_scripts_styles', 100);
...
function g365_add_event_id_reference() {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/ss-press/css/', 'https' ) . "style-admin.css?ver=2.4" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://sportspassports.com/data-processor.js' : 'https://dev.sportspassports.com/data-processor.js'), array('admin-jquery'), '695450', true );
...
function ogp_cart_process(){
  ...
  if( !empty(WC()->cart->get_cart()) ) {
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
      ...
      $product_var_name = apply_filters( 'woocommerce_cart_item_product_name', $cart_item['variation']['attribute_level'], $cart_item, $cart_item_key );
      ...
      $extra_types = array( 'fee', 'playerclub', 'gear', 'trainingdays', 'trainingdaysladera', 'event', 'newclub', 'campsupdated', 'findus', 'highsch', 'ybl', 'perfrecovery', 'shirt', 'laderalunch', 'anaheimlunch' );
        ...
        if( in_array($product_cat_slug, $extra_types) ) {
          switch( $product_cat_slug ) {
            ...
            case 'playerclub':
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'select',
                  'placeholder'=> 'Players Uniform Size',
                  'label' => 'Player Uniform ' . $i,
                  'name'  => 'player_uniform_size_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                  'options' => array(
                    'y_sm' => 'Youth Small',
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                $extra_fields[] = array(
                  ...
                  'name'  => 'player_name_' ,
                  'required'=> true,
                  'class' => array($product_cat_slug, 'player-name-ybl')
                );
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'title' => $product_name,
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'number',
                  'placeholder'=> 'Players Height - Inches',
                  'label' => 'Player Height ' ,
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Player request, Coach request, Practice request, etc.',
                  'label' => 'Special Request ' ,
                  'name'  => 'special_request_' ,
                  'required'=> false,
                  'class' => array($product_cat_slug, 'special-request')
                 ); 
                $extra_fields[] = array(
                  'type'  => $product_cat_slug,
                  'title' => $product_name,
                  'type'  => 'select',
                  'placeholder'=> 'Please Select One if ',
                  'label' => 'Interested in Volunteering?' ,
                  'name'  => 'volunteer' ,
                  'required'=> false,
                  'class' => array($product_cat_slug, 'end-multi-player'),
                  'options' => array(
                    'placeholder' => 'volunteer option',
                    'coach' => 'Head Coach',
                    'none' => 'Not Interested'
                  )
                 );
            ...
            case 'gear':
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                $extra_fields[] = array(
                  'label' => 'Monday: 4:00 PM – 5:00 PM (*12 & Under) ',
                  'name'  => 'monday:_4_–_5_PM_(*12_&_Under)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom', 'performance-training-title'),
                );
                $extra_fields[] = array(
                  'label' => 'Monday: 5:00 PM – 6:00 PM (*All Ages)',
                  'name'  => 'monday:_5_–_6_PM_(*12_&_Under)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Monday: 6:00 PM – 7:00 PM (*13 & Over)',
                  'name'  => 'monday:_6_–_7_PM_(*13_&_Over)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Tuesday: 5:00 PM – 6:00 PM (*13 & Over) ',
                  'name'  => 'tuesday:_5_–_6_PM_(*12_&_Under)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Tuesday: 6:00 PM – 7:00 PM (*13 & Over) ',
                  'name'  => 'tuesday:_6_–_7_PM_(*13_&_Over)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Wednesday: 5:00 PM – 6:00 PM (*All Ages) ',
                  'name'  => 'wednesday:_5_–_6_PM_(*12_&_Under)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                 $extra_fields[] = array(
                  'label' => 'Wednesday: 6:00 PM – 7:00 PM (*13 & Over) ',
                  'name'  => 'wednesday:_6_–_7_PM_(*13_&_Over)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                 $extra_fields[] = array(
                  'label' => 'Wednesday: 7:00 PM – 8:00 PM (*13 & Over) ',
                  'name'  => 'wednesday:_7_–_8_PM_(*13_&_Over)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Thursday: 5:00 PM – 6:00 PM (*13 & Over) ',
                  'name'  => 'thursday:_5_–_6_PM_(*12_&_Under)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Thursday: 6:00 PM – 7:00 PM (*13 & Over) ',
                  'name'  => 'thursday:_6_–_7_PM_(*13_&_Over)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
              ...
              case 'trainingdaysladera':
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
//                 $extra_fields[] = array(
//                   'label' => 'Tuesday: 5:00 PM – 6:00 PM (*12 & Under) ',
//                   'name'  => 'tuesday:_5_–_6_PM_(*12_&_Under)_',
//                   'type'  => 'checkbox',
//                   'required'=> false,
//                   'class' => array($product_cat_slug, 'medium-margin-bottom', 'performance-training-title'),
//                 );
                $extra_fields[] = array(
                  'label' => 'Monday: 6:00 PM – 7:00 PM (*12u-15u)',
                  'name'  => 'monday:_6_–_7_PM_(*12u_-_15u)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Monday: 7:00 PM – 8:00 PM (*16u+) ',
                  'name'  => 'monday:_7_–_8_PM_(*16u+)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                ...
                $extra_fields[] = array(
                  'label' => 'Wednesday: 6:00 PM – 7:00 PM (*12u-15u)',
                  'name'  => 'wednesday:_6_–_7_PM_(*12u_-_15u)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Wednesday: 7:00 PM – 8:00 PM (*16u+) ',
                  'name'  => 'wednesday:_7_–_8_PM_(*16u+)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
...
//                 );
//                 $extra_fields[] = array(
//                   'label' => 'Tuesday: 7:00 PM – 8:00 PM (*13 & Over) ',
//                   'name'  => 'tuesday:_7_–_8_PM_(*13_&_Over)_',
//                   'type'  => 'checkbox',
//                   'required'=> false,
//                   'class' => array($product_cat_slug, 'medium-margin-bottom'),
//                 );
                ...
                $extra_fields[] = array(
                  'label' => 'Thursday: 5:00 PM – 6:00 PM (*Open Age Group) ',
                  'name'  => 'thursday:_5_–_6_PM_(*Open_Age_Group)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Thursday: 6:00 PM – 7:00 PM (*Open Age Group) ',
                  'name'  => 'thursday:_6_–_7_PM_(*Open_Age_Group)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
                $extra_fields[] = array(
                  'label' => 'Thursday: 7:00 PM – 8:00 PM (*Open Age Group) ',
                  'name'  => 'thursday:_7_–_8_PM_(*Open_Age_Group)_',
                  'type'  => 'checkbox',
                  'required'=> false,
                  'class' => array($product_cat_slug, 'medium-margin-bottom'),
                );
            ...
            case 'highsch':
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                $extra_fields[] = array(
                  ...
                  'label' => 'Highschool Name ' . $i,
                  'name'  => 'highschool_name_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
              }
             ...
             case 'shirt':
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Player Division',
                  'label' => 'Player Division ',
                  'name'  => 'player_division_',
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'select',
                  'placeholder'=> 'Parent Shirt Size',
                  'label' => 'Parent Shirt Size ',
                  'name'  => 'parent_shirt_size_',
                  'required'=> true,
                  'class' => array($product_cat_slug),
                  'options' => array(
                    'small' => 'Adult Small',
                    'medium' => 'Adult Medium',
                    'large' => 'Adult Large',
                    'xlarge' => 'Adult X-Large',
                    '2xlarge' => 'Adult XX-Large',
                    '3xlarge' => 'Adult XXX-Large'
                  )
                );
              }
              break;
              ...
              for ($i = 1; $i <= $cart_item['quantity']; ++$i) {
                ...
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'text',
                  'placeholder'=> 'Please list any allergy/dietary restrictions',
                  'label' => 'Please list any allergy/dietary restrictions ' . $i,
                  'name'  => 'please_list_any_allergy/dietary_restrictions_' . $i,
                  'required'=> true,
                  'class' => array($product_cat_slug)
                );
                $extra_fields[] = array(
                  ...
                  'title' => $product_name,
                  'type'  => 'select',
                  'placeholder'=> 'Select Attending Camp',
                  'label' => 'Select Attending Camp ',
                  'name'  => 'select_attending_camp_',
                  'required'=> true,
                  'class' => array($product_cat_slug),
                  'options' => array(
                    'June1923' => 'June 19-23',
    ...
    $extra_fields[] = array(
      ...
      'type'    => 'select',
      'require'  => false,
      'class'   => array('input', 'find-us-checkout'),
...
add_filter( 'woocommerce_before_cart' , 'ogp_cart_setup');
function ogp_cart_setup() {
  //load in global proc var
  global $cart_parse;
  //process cart items
  $cart_parse = ogp_cart_process();
  ...
  if( !empty($cart_parse[2]) ){
    ...
    add_filter( 'woocommerce_after_order_notes' , function( $fields ) use ( $order_data_field ) { return ogp_player_checkout_field( $field, $order_data_field ); });
  }
}

//make the admin credential string
function g365_make_admin_key() {
  $grassroots_keys = get_option( 'ogp_g365_connector' );
  $current_user = wp_get_current_user();
...
function g365_order_data_form_field($no_parameter, $key, $args, $value) {
  ...
  foreach( $args['g365_cart_types'] as $dex => $type ){
    switch( $type ){
      ...
      case 'club_team':
        $current_user = wp_get_current_user();
        $presets[] = $type . '_preset:::user_ac::' . ((strpos(site_url(), 'dev') === false) ? 'OGP' : 'OGD') . '-' . $current_user->ID;
        break;
    }
  }
...
function ogp_player_checkout_field( $checkout, $extra_fields = null ) {
  ...
  echo '<input type="hidden" name="ogp_extra_data_required" value="' . implode(',',$added_field_names_required) . '" />';
  echo '<input type="hidden" name="ogp_extra_data" value="' . implode(',',$added_field_names) . '" />';
}

//make the extra data required based on the field names variable
add_action('woocommerce_checkout_process', 'ogp_player_checkout_field_process');
function ogp_player_checkout_field_process() {
  // Check if we have extra data
  if ( $_POST['ogp_extra_data_required'] ) {
    //parse and loop through the set to make sure we have all the fields filled out
    $fields = explode(',', $_POST['ogp_extra_data_required']);
...
add_action( 'woocommerce_checkout_update_order_meta', 'ogp_player_checkout_field_update_order_meta' );
function ogp_player_checkout_field_update_order_meta( $order_id ) {
  //join the extra_data list if we need to
  $ogp_extra_data = array();
  $ogp_extra_data_fields = array();
  if ( $_POST['ogp_extra_data'] ) {
    $ogp_extra_data[] = $_POST['ogp_extra_data'];
    $ogp_extra_data_fields[] = $_POST['ogp_extra_data'];
  }
  if ( $_POST['ogp_extra_data_required'] ) {
    $ogp_extra_data[] = $_POST['ogp_extra_data_required'];
    $ogp_extra_data_fields[] = $_POST['ogp_extra_data_required'];
  }
  //save the field reference so we know what to pull later
  if ( !empty($ogp_extra_data) ) update_post_meta( $order_id, 'ogp_extra_data', sanitize_text_field( implode(',', $ogp_extra_data) ) );
  // Check if we have extra data
  if ( !empty($ogp_extra_data_fields) ) {
    //parse extra field names, then loop and save
    $fields = explode(',', implode(',', $ogp_extra_data_fields));
    foreach( $fields as $dex => $key ) {
...
add_action( 'woocommerce_admin_order_data_after_billing_address', 'ogp_player_checkout_field_display_admin_order_meta', 10, 1 );
function ogp_player_checkout_field_display_admin_order_meta($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'ogp_extra_data', true );
  if( !empty($fields) ) {
...
function ogp_edit_memberships_actions( $actions ) {
...
add_filter( 'wc_memberships_members_area_my-memberships_actions', 'ogp_edit_memberships_actions' );
add_filter( 'wc_memberships_members_area_my-membership-details_actions', 'ogp_edit_memberships_actions' );
...
function add_series_options() {
  ...
  if( !empty( $product_ids ) && is_array($product_ids) ) {
    ...
    foreach ( $product_ids as $product_id ) {
      ...
      //item prices
      $series_prices[ $product_id ] = str_replace( '.00', '', $series_products[ $product_id ]->get_price_html() );
      //get event_link
    ...
    foreach ( $product_ids as $product_id ) {
      ...
      if( !empty($series_events[ $product_id ][ 'data_pull' ]) ) {
        ...
        if( empty($series_events[ $product_id ][ 'data_pull' ]->dates) ) {
          ...
          //add to the series order so we can be organized
          $series_order[ $product_id ] = date( 'Y-m-d', strtotime(ogp_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 4)));
          ...
          if( $payment_options ) {
            ...
            $series_events[ $product_id ][ 'label' ][] = ogp_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 1, true);
...
//       echo '<label class="add_upsell_product" for="check_'. $add_upsell_product->get_id() . '"><input type="checkbox" name="additional-add-to-cart[]" id="check_'. $add_upsell_product->get_id() . '" value="'. $add_upsell_product->get_id() . '"> ' . $add_upsell_product->get_title() . ': $' . $add_upsell_product->get_price() . '</label>';
      echo '<label class="add_upsell_product" for="check_'. $add_upsell_product->get_id() . '"><input type="checkbox" name="additional_add_to_cart" id="check_'. $add_upsell_product->get_id() . '" value="'. $add_upsell_product->get_id() . '"> ' . $add_upsell_product->get_title() . ': $' . $add_upsell_product->get_price() . '</label>';
...
add_filter( 'woocommerce_checkout_fields' , 'ogp_modify_checkout_fields' );
// Our hooked in function - $fields is passed via the filter!
function ogp_modify_checkout_fields( $fields ) {
   unset($fields['order']['order_comments']);
   return $fields;
}

//validate the checkout form before finishing the process of writting data to ogp about the purchase
add_action('woocommerce_after_checkout_validation', 'process_ogp_data');
function process_ogp_data($form_data) {
...
add_action( 'woocommerce_checkout_update_order_meta', 'ogp_update_meta_form_fields' );
function ogp_update_meta_form_fields( $order_id ) {
...
add_action( 'show_user_profile', 'ogp_user_fields_display' );
add_action( 'edit_user_profile', 'ogp_user_fields_display' );

function ogp_user_fields_display( $user ) {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/ss-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://sportspassports.com/data-processor.js' : 'https://dev.sportspassports.com/data-processor.js'), array('admin-jquery'), '695450', true );
  ?>
	<h3><?php esc_html_e( 'Additional User Data', 'ogp' ); ?></h3>
  <div class="options_group g365_manage_wrapper">
...
add_action( 'user_profile_update_errors', 'ogp_user_field_error', 10, 3 );
function ogp_user_field_error( $errors, $update, $user ) {
	if ( ! $update ) {
		return;
	}
}

add_action( 'personal_options_update', 'ogp_user_fields_update' );
add_action( 'edit_user_profile_update', 'ogp_user_fields_update' );

function ogp_user_fields_update( $user_id ) {
...
*/
  add_action('woocommerce_cart_calculate_fees', function(){
    if( !empty(WC()->cart->get_cart()) ){
      // Check YBL and Youth Camps products in cart
      foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ){
        //root product id
        $product_id = apply_filters( 'woocommerce_cart_item_product_id', $cart_item['product_id'], $cart_item, $cart_item_key );
        //form type is tied to product categories, we only grab the first cat
        $product_cats = get_the_terms( $product_id, 'product_cat' );
        $extra_types = array( 'campsupdated', 'ybl' );
        $product_cat_slug = $product_cats[0]->slug;
        if( in_array($product_cat_slug, $extra_types) ){
          if (is_admin() && !defined('DOING_AJAX')){ return; }
          $percentage = 0.029;
          $percentage_fee = (WC()->cart->get_cart_contents_total() + WC()->cart->get_shipping_total()) * $percentage;
           WC()->cart->add_fee(__('A 2.9% merchant fee is applied to all credit card transactions. We accept Visa / MasterCard / American Express / Discover', 'txtdomain'), $percentage_fee);
        }
      }
    }
  });
